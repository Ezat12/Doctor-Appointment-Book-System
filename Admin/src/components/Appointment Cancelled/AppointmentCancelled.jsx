...
function AppointmentsCancelled() {
    ...
    <div className="appointment">
      ...
      <table className="w-full mt-4 flex flex-col">
        <thead className="bg-white py-3 px-5 flex">
          ...
          <th className="flex-1 text-end">Actions</th>
        </thead>
        <tbody>
          {loadingAppointment ? (
          ...
          ) : appointment.filter((app) => app.status === "Cancelled").length >
            0 ? (
            appointment.map(
              (app, index) =>
                app.status === "Cancelled" && (
                  ...
                  >
                    ...
                    <td className="flex-1 text-start">
                      <span className="text-red-500">{app.status}</span>
                    </td>
                    <td className="flex-1 flex items-center justify-end">
                      {app.is_paid ? (
                        <div className="flex items-center">
                          <div className="group">
                            <button className="border rounded-md py-2 px-10 transform duration-200 text-gray-600 flex gap-4 items-center justify-center hover:text-white hover:border-red-500 hover:bg-red-500">
                              Paid
                              <PiChecksBold
                                size={"25px"}
