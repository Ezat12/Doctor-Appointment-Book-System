...
function MyAppointment() {
  ...
  const handlePaidOnline = async (appointmentId, appointmentFee) => {
    setLoadingPayment(true);
    const checkoutResponse = await axios.post(
      `${import.meta.env.VITE_BASE_URL_SERVER}/payment/create-checkout-session`,
      { appointmentId, amount: appointmentFee },
      { headers: { Authorization: `Bearer ${Cookies.get("auth-token")}` } }
    );
    setLoadingPayment(false);

    const url = checkoutResponse.data.session.url;
    location.href = url;
  };
  ...
  return (
    <>
      {loadingPayment && (
        <div className="fixed top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] bg-[#e5e7eb5e] w-full h-full flex items-center justify-center">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-6">
                    ...
                    <div className="flex flex-col space-y-2 md:w-48">
                      ...
                      {appointment.status === "Completed" && (
                        <button
                          // onClick={() =>
                          //   navigate(`/feedback/${appointment._id}`)
                          // }
